...
if (isset($_POST['all'])) {
    ...
    try {
        ...
        $userActions = $dbObj->SelectPrepared("SELECT id_dnevnik, k.korisnicko_ime, tp.naziv as naziv_radnje, d.* FROM dnevnik as d INNER JOIN korisnik k ON k.id_korisnik = d.id_izvrsitelj INNER JOIN tip_radnje tp ON tp.id_tip = id_radnja ORDER BY datum_vrijeme DESC LIMIT $offset, $configItemsPerPage");
...
if (isset($_POST['username'])) {
    ...
    try {
        $offset = $currentPage * $configItemsPerPage;
        $userActions = $dbObj->SelectPrepared("SELECT k.korisnicko_ime, tp.naziv as naziv_radnje, d.* FROM dnevnik as d INNER JOIN korisnik k ON k.id_korisnik = d.id_izvrsitelj INNER JOIN tip_radnje tp ON tp.id_tip = id_radnja WHERE d.id_izvrsitelj = ? LIMIT $offset, $configItemsPerPage", "i", [$userInfo->id_korisnik]);
        die(json_encode($userActions));
    } catch (Exception $e) {
        die(json_encode(-1));
    }
