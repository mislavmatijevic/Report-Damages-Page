<section class="section">
    ...
    <form id="login" name="login" method="POST" class="section-login_form" action="{htmlspecialchars($smarty.server.PHP_SELF)}">
        ...
        <input id="username" name="username" {if isset($loginUser["username"])} value="{htmlspecialchars($loginUser["username"])}" {/if} />
        <span id="error-username" class="error"></span>
        <label for="password" class="section-login_form-password">Lozinka: </label>
        <input id="password" name="password" type="password" {if isset($loginUser["password"])} value="{htmlspecialchars($loginUser["password"])}"
            {/if} />
