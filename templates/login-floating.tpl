<section class="section-login">
    <form id="login" name="login" method="POST" class="section-login_popup" action="{$smarty.server.PHP_SELF}">
        ...
        <input id="username" name="username" placeholder="Korisničko ime" {if isset($loginUser)} value="{$loginUser["username"]}"
            {/if} />
        <span id="error-username" class="error"></span>
        <input id="password" name="password" type="password" placeholder="Password" {if isset($loginUser)}
